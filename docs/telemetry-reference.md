...
| QueryClient | `@tanstack/react-query` via `router.tsx` |
...
```text
1. Browser tab opens → SSR page request (no session header)          [browser-session-client.ts]
2. Server creates browser session UUID → HTML <meta> + response hdr  [browser-session.ts, request.ts]
3. Client hydrates → sessionStorage; fetch patch adds session header [install-client-telemetry-fetch.ts]
4. handleInstrumentedRequest → page span GET /explore + metrics      [request.ts]
5. beforeLoad (instrumentBeforeLoad) → span beforeLoad./explore      [helpers.ts]
6. Browser calls /_serverFn with session id header                   [install-client-telemetry-fetch.ts]
7. telemetryFunctionMiddleware → auth.sessionCheck / serverFn span   [middleware.ts]
8. axios GET /users/logged → apiClient.GET.users.logged + metrics    [axios-hooks.ts]
9. Spans export to Azure Monitor under shared operation_Id           [setup.ts]
```
...
└─ beforeLoad./explore                ← route guard
   └─ auth.sessionCheck / serverFn.*  ← server function
...
| `src/server/telemetry/middleware.ts` | —       | Creates serverFn spans, records `tanstack.server_fn.*` metrics |

### Layer 4 — Outbound dependencies

| File                                  | Lines   | Role                                                                              |
| ------------------------------------- | ------- | --------------------------------------------------------------------------------- |
| `src/server/telemetry/axios-hooks.ts` | —       | Global axios interceptors: trace propagation, `apiClient.*` spans, client metrics |
...
### Layer 5 — Client propagation (no OTel imports)

| File                                                             | Lines   | Role                                                               |
| ---------------------------------------------------------------- | ------- | ------------------------------------------------------------------ |
| `src/router.tsx`                                                 | —       | Installs fetch patch + plain QueryClient at router creation        |
| `src/modules/shared/telemetry/install-client-telemetry-fetch.ts` | —       | Patches `globalThis.fetch` — session id header only                |
...
| Metric                            | Type           | Key attributes                                                         | Source |
| --------------------------------- | -------------- | ---------------------------------------------------------------------- | ------ |
| `http.client.request.duration`    | Histogram (ms) | `method`, `resource`, `status_class`, `browser_session_id` | `registry-factory.ts:69-74`, `151-179` |
| `http.client.dependency.duration` | Histogram (ms) | `status_class`                                                         | `registry-factory.ts:58-61`, `180-183` |
| `http.client.errors`              | Counter        | `status_class`                                                         | `registry-factory.ts:62-63`, `184-189` |
...
| --- | --- | --- |
| `browser.session.id` | Tab UUID | browser-session |
| `correlation.id` | Page request id | request / helpers |
| `http.status_code` | Set on response | registry-factory |
...
| Server functions | `middleware.ts` via `start.ts` |
| Axios outbound | `axios-hooks.ts` via `registry-factory.ts` |
| Browser fetch (session id) | `install-client-telemetry-fetch.ts` via `router.tsx` |

### Manual — new route with data

Wrap `beforeLoad` with `instrumentBeforeLoad` — `helpers.ts`; example `auth.tsx`.

### Manual — server metric

`recordMetric` / `recordAuthRedirect` — `helpers.ts`.
